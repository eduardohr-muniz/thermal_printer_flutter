...
- Android Bluetooth writes now run on a background thread (in chunks, no delays) instead of blocking the platform/UI thread. This removes the freeze/jank (and ANR risk) during long prints; raw SPP throughput is unchanged (it is gated by the printer, not the plugin).
